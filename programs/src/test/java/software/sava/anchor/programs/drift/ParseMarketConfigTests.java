...
import static software.sava.anchor.programs.drift.GenerateMarketConstants.MAIN_NET_PERP_KEY;
import static software.sava.anchor.programs.drift.GenerateMarketConstants.quoteJson;

final class ParseMarketConfigTests {

  @Test
  void parsePerpMarketsTest() {
    final var ts = """
    ...
    var response = GenerateMarketConstants.convertJson(ts);

    int from = response.indexOf(MAIN_NET_PERP_KEY);
    from = response.indexOf('[', from + MAIN_NET_PERP_KEY.length());
    int to = response.indexOf("];", from) + 1;

    final var mainNetJson = quoteJson(response.substring(from, to));
    var ji = JsonIterator.parse(mainNetJson);
    final var mainNetConfigs = PerpMarketConfig.parseConfigs(ji, DriftAccounts.MAIN_NET);
  }
}
